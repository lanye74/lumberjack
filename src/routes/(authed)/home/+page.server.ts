...
export const actions = {
	submitLocation: async (requestEvent) => {
		...
		const {data: readPointsData, error: readPointsError} = await supabase.from("ast_public_user_data")
			.select("points")
		...
		const {error: updatePointsError} = await supabase.from("ast_public_user_data")
